...
batch_size = 8           # 【修改1】从 64 改为 8，适应小数据量
num_epochs = 20          # 【修改2】从 60 改为 20，快速验证流程
...
if __name__ == "__main__":
    ...
    # ==========================================
    # 【修改3】核心保命逻辑：如果只有一个文件，既当训练集又当验证集
    # ==========================================
    if len(all_pkl_files) == 1:
        print("\n[!] 警告：只检测到 1 个数据包！")
        print("[!] 开启“单样本过拟合测试”模式：训练集和验证集使用同一份数据。")
        train_files = all_pkl_files
        val_files = all_pkl_files
    else:
    ...
    # 注意：num_workers 如果在资源紧张时报错，可以改成 0
